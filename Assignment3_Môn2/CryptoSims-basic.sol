...
contract TheFunixCryptoSim is ERC721, Ownable {
    ...
    function createGenesis() internal {
        ...
        sims.push(Sim({attributes: firstAtrributes, matronId: 0, sireId: 0}));

        super._mint(msg.sender, 0);

        sims.push(Sim({attributes: secondAtrributes, matronId: 0, sireId: 0}));
    ...
    function getSimDetails(
        uint256 simId
    ) external view returns (uint256, SimAttributes memory, uint256, uint256) {
